...
int minimax(char a[23][45], int alpha, int beta, int depth, bool isMax)
{
    ...
    if (isMax)
    {
        ...
        for (int i = 0; i<10; i++)
        {
            for (int j = 0; j<10; j++)
            {
            	// Kiem tra xem o co trong hay khong
                if (a[i*2+3][j*4+6]==' ')
                {
                	// Thuc hien nuoc di
                    a[i*2+3][j*4+6] = 'X';
...
Move findBestMove(char a[23][45])
{
    ...
    int alpha = -10000000;
    int beta = 10000000;
 	// Lan luot do qua tat ca cac o
 	// Tien hanh danh gia gia tri ham minimax cho tat ca cac o trong
 	// Tra ra nuoc di co gia tri toi uu nhat
    
    for (int i = 0; i<10; i++)
    {
        for (int j = 0; j<10; j++)
        {
            // Kiem tra o co trong hay khong
            if (a[i*2+3][j*4+6]==' ')
            {
                // Tien hanh danh nuoc di
                a[i*2+3][j*4+6] = 'X';
 
                // Bot tinh toan gia tri cua nuoc di
                int moveVal = minimax(a, alpha, beta, 0, false);
 
                // di lai nuoc di (undo the move)
                a[i*2+3][j*4+6] = ' ';
 
                // Kiem tra gia tri nuoc di hien tai
                // neu gia tri nuoc di hien tai la tot nhat thì tien hanh thay the
                if (moveVal > bestVal)
                {
                    bestMove.rw = i*2+3;
                    bestMove.cl = j*4+6;
...
int main(){
	...
	do{
	    ...
	    switch(number)
		{
			...
			case 5:
				cout<<"----------------------------Guide----------------------------"<<endl;
				cout<< "Nguoi choi lan luot thuc hien cac nuoc di bang cach nhap vao tu ban phim (Hang,Cot)" <<endl;
				cout<< "Khong duoc nhap vao o khong hop le nhu la o da duoc nhap hoac o khong co tren ban co"<<endl;
				cout<< "Dieu kien thang: xuat hien 4 quan X hoac O lien tiep theo hang,cot,duong cheo" <<endl;
				break;
    		...
    		case 2:
				...
				{
					case 2:
						cout<<"!!!!!!!! Che do se co trong tuong lai !!!!!!!!"<<endl;
						break;
					case 3:
						cout<<"!!!!!!!! Che do se co trong tuong lai !!!!!!!!"<<endl;
						break;
			...
			case 4:
    			cout << endl << "*-------Player's Information'-------*" << endl;
            	cout << "1.Newest Information" << endl;
            	cout << "2.History" << endl;
            	cout << "Press number to choice: ";
            	cin >> number;
            	switch(number)
